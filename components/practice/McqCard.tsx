    ...
    <Animated.View style={animatedStyle}>
      <View className="bg-surface dark:bg-slate-800 border border-border-subtle dark:border-slate-700 p-5 rounded-2xl mb-4">
        <View className="flex-row items-center justify-between mb-3">
          <Text className="text-clinical-blue dark:text-sky-400 font-extrabold text-xs uppercase tracking-wider">
            📝 Scenario MCQ Practice
          </Text>
          <Pill label="Standard" variant="info" size="sm" />
        </View>

        {/* Clinical Scenario Vignette */}
        {question.scenarioText && (
          <View className="bg-slate-50 dark:bg-slate-900/90 p-4 rounded-xl border border-border-subtle dark:border-slate-800 mb-4">
            <Text className="text-slate-500 dark:text-slate-400 text-[10px] font-extrabold uppercase mb-1">
              Clinical Vignette
            </Text>
            <Text className="text-navy dark:text-slate-200 text-xs leading-5">
              {question.scenarioText}
            </Text>
          </View>
        )}
        ...
        <View className="space-y-3">
          {question.options.map((opt, idx) => {
            ...
            if (isAnswered) {
              ...
              cardBg = "bg-clinical-blue-light dark:bg-sky-950/90 border-clinical-blue dark:border-sky-400";
              textColor = "text-clinical-blue dark:text-sky-200 font-bold";
            }

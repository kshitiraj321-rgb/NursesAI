...
export default function QuizView({ questions, loading, onExit, onComplete, type }: QuizProps) {
  ...
  const handleNext = () => {
    ...
    const currentTopic: string = (questions[current] as any).topic || "General";

    let newWeakTopics = { ...weakTopics };
    let newStrongTopics = { ...strongTopics };

    if (isCorrect) {
      newStrongTopics[currentTopic] = (newStrongTopics[currentTopic] || 0) + 1;
    } else {
      newWeakTopics[currentTopic] = (newWeakTopics[currentTopic] || 0) + 1;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {type === "pyq" && questions[current]?.year && (
          <Text
            style={styles.pyqMeta}
          >
            NORCET {questions[current].year} • PYQ
          </Text>
        )}
        ...
        {questions[current].options.map((opt, i) => {
          ...
          if (isPyq && selected) {
            if (isCorrect) {
              optionStyle = isSelected ? styles.optionButtonSelectedCorrect : styles.optionButtonDefault;
            } else {
              if (isSelected) {
                optionStyle = styles.optionButtonSelectedWrong;
              } else if (isCorrectOption) {
                optionStyle = styles.optionButtonRevealCorrect;
              }
            }
          } else if (isSelected) {
        ...
        {type === "pyq" && selected && (
          <Text style={styles.explanationText}>
            Explanation: {questions[current]?.explanation || "No explanation available"}
          </Text>
        )}
      </ScrollView>
      ...
      <TouchableOpacity onPress={onExit} style={styles.exitButton}>
        <Text style={styles.exitText}>← Exit Quiz Modes</Text>
      </TouchableOpacity>
    </View>
  );
}
